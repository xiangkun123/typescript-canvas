 ...
 */
function step3(timestamp: number): void {
    // 第一次调用函数时，设置step3和lastTime3为timestamp
    if( !step3 ) start3 = timestamp;
    if( !lastTime3 ) lastTime3 = timestamp;

    // 计算当前时间点与第一次调用step3时间点的差
    let elapsedMsec: number = timestamp - start3;

    // 两帧之间的时间差
    let intervalMsec: number = timestamp - lastTime3;
